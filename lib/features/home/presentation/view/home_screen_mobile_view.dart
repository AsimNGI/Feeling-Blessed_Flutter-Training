...
import 'package:flutter_training/features/home/presentation/widgets/generic_section.dart';
import 'package:flutter_training/features/home/presentation/widgets/header_section.dart';
import 'package:flutter_training/features/home/presentation/widgets/welcome_section.dart';
...
class HomeScreenMobileView extends StatefulWidget {
  ...
  @override
  State<HomeScreenMobileView> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<HomeScreenMobileView> {
  @override
  void initState() {
    super.initState();
      ...
      body: BlocBuilder<HomeBloc, HomeState>(
        builder: (context, state) {
          if (state is HomeLoading || state is HomeInitial) {
            return const Center(child: CircularProgressIndicator(color: AppColors.primaryAqua600));
          }
  ...
  /// Convert API [HomeResponse] to [NewHomeResult] for existing section widgets.
  NewHomeResult _toNewHomeResult(HomeResponse data) {
    return NewHomeResult(
      banner: data.banner,
      sections: data.sections.map(_toNewHomeSection).toList(),
      onBoardingSection: data.onBoardingSection
          .map((o) => OnboardingSectionItem(identifier: o.identifier, visibility: o.visibility))
          .toList(),
    );
  }
